...
# ---------------------------
# functie trimitere in ArcGIS Online
# ---------------------------
...
with col2:
    ...
    # marker-ele sesizarilor deja salvate local
...
if st.button("Trimite sesizarea"):
    if not descriere.strip():
        ...
        if "addResults" in rezultat_arcgis and rezultat_arcgis["addResults"][0].get("success") is True:
            ...
            #st.write("Raspuns ArcGIS:", rezultat_arcgis)
        else:
            st.warning("Sesizarea a fost salvata local, dar nu a fost trimisa corect in ArcGIS Online.")
            st.write("Raspuns ArcGIS:", rezultat_arcgis)
...
#if os.path.exists(FISIER_DATE):
#   st.markdown("---")
#    st.subheader("Sesizari salvate local")
